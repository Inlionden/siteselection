    ...
    for card in cards:
        ...
        try:
            ...
            if card.name == 'a' and card.get('href'):
                link = card['href']
                # Try aria-label first for name
                name = card.get('aria-label') or card.get_text(strip=True)[:200]
            else:
                link_tag = card.find('a', href=True)
                link = link_tag['href'] if link_tag else ''
                # name often in div with role heading inside the card
                # Try common classes for name
                name_tag = card.find('div', class_='qBF1Pd') or \
                           card.find('div', class_='fontHeadlineSmall') or \
                           card.find(['div', 'h3'], recursive=True)
            ...
            if name: # Only add if we managed to get a name
                candidates.append({
                    "name": name,
                    "link": link,
                    "rating": rating,
